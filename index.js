...
module.exports = function (filename, options) {
    ...
    if(options.packageRoot) {
        pathTransform = function(path) {
            return options.packageRoot + (options.packageRoot.endsWith('/') ? path : '/' + path);
        }
        options.packageRoot.split('/').reduce(function(path, part) {
            if(part == '') {
                return path;
            }
        ...
        pathTransform = function(path) { return path; };
    }
    ...
    return through.obj(function (file, enc, cb) {
        ...
        if(file.stat.isSymbolicLink()) {
            archive.entry({
                name: pathTransform(file.relative.replace(/\\/g, '/') + (file.isNull() ? '/' : '')),
                mode: file.stat.mode,
                type: 'symlink',
